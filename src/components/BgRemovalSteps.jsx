...
// Steps for the background removal process
const steps = [
  {
    step: "Step 1",
...
const BgRemovalSteps = ({ scrollToRef }) => {
  const [activeStep, setActiveStep] = useState(0); // Tracks the current active step
  const [isVisible, setIsVisible] = useState(false); // Handles animation visibility

  // Auto-cycling steps for animation effect
  useEffect(() => {
    setIsVisible(true);
    ...
    <div className="relative overflow-hidden rounded-3xl shadow-lg bg-white dark:bg-gray-900">
      ...
      <div className="absolute inset-0 bg-gradient-to-br from-slate-50 via-white to-blue-50">
        <div
          ...
          style={{
            backgroundImage:
              "url(\"data:image/svg+xml,%3Csvg width='60' height='60' viewBox='0 0 60 60' xmlns='http://www.w3.org/2000/svg'%3E%3Cg fill='none' fill-rule='evenodd'%3E%3Cg fill='%23f1f5f9' fill-opacity='0.4'%3E%3Ccircle cx='7' cy='7' r='1'/%3E%3C/g%3E%3C/g%3E%3C/svg%3E\")",
          }}
        ></div>
      </div>
      ...
      <div className="relative px-4 sm:px-6 md:px-8 lg:px-12 xl:px-24 py-16 sm:py-20 lg:py-24">
        ...
        <div className="text-center mb-12 sm:mb-16 lg:mb-20">
          ...
          <h2
            className={`text-2xl sm:text-3xl md:text-4xl lg:text-5xl xl:text-6xl font-extrabold bg-gradient-to-r from-gray-900 via-gray-800 to-gray-900 bg-clip-text text-transparent mb-4 sm:mb-6 leading-tight transition-all duration-1000 ${
          ...
          <p
            className={`text-base sm:text-lg lg:text-xl text-gray-600 max-w-3xl mx-auto leading-relaxed transition-all duration-1000 delay-200 ${
              isVisible ? 'translate-y-0 opacity-100' : 'translate-y-10 opacity-0'
            }`}
          >
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6 mb-12 sm:mb-16">
          {features.map((feature, index) => (
            <div
              key={index}
              className={`bg-white/70 backdrop-blur-sm rounded-2xl p-4 sm:p-6 text-center shadow-lg hover:shadow-xl transition-all duration-300 hover:-translate-y-1 ${
                isVisible ? 'translate-y-0 opacity-100' : 'translate-y-10 opacity-0'
              }`}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 sm:gap-8 lg:gap-10 mb-12 sm:mb-16">
          {steps.map((item, index) => {
            ...
            return (
              <div
                key={index}
                className={`group relative bg-white/80 backdrop-blur-sm rounded-3xl p-6 sm:p-8 shadow-lg hover:shadow-2xl transition-all duration-500 cursor-pointer border border-white/20 ${
