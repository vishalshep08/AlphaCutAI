...
const Pricing = () => {
  ...
  // Local state for billing cycle and loading status
  const [billingCycle, setBillingCycle] = useState("monthly");
  const [isLoading, setIsLoading] = useState(false);

  /**
   * Handles placing an order for a plan.
   * Shows sign-in modal if user is not signed in.
   * Calls backend API to place the order and handles errors.
   */
  const handleOrder = async (planId) => {
    ...
    try {
      ...
      if (status === 409) {
        ...
        toast.error("You already purchased this plan."); // Handle DB conflict error
      } else {
  ...
  /**
   * Calculate savings percentage between original price and discounted price.
   */
  ...
  // If user is not signed in, show sign-in prompt UI
  if (!isSignedIn) {
    return (
  ...
  // Main Pricing UI when signed in
  return (
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="text-center mb-12 sm:mb-16">
            ...
            <p className="text-lg sm:text-xl text-gray-300 max-w-3xl mx-auto leading-relaxed mb-8">
              Select from our carefully curated packages designed to meet your specific needs and
              budget. Start free, scale as you grow.
            </p>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 lg:gap-8 mb-16">
            {plans.map((plan) => {
              ...
              const displayOriginalPrice = plan.originalPrice
                ? getPriceByBilling(plan.originalPrice)
                : null;
              ...
              return (
                <div
                  ...
                  className={`relative group ${
                    plan.popular ? "scale-105 lg:scale-110 z-10" : "hover:scale-105 z-0"
                ...
                >
                  ...
                  <div
                    className={`relative h-full bg-gray-800/50 backdrop-blur-xl rounded-2xl border transition-all duration-300 ${
                      plan.popular
