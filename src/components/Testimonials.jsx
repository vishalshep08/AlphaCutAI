...
export const initialTestimonials = [
  ...
  },
  {
    id: 2,
    quote:
      "AlphaCut.AI is leaps and bounds ahead of the competition. A thousand times better. It simplified the whole process.",
...
// Star rating component for displaying and optionally selecting rating
...
// Card component to display each testimonial
const TestimonialCard = ({ testimonial, isNew = false }) => {
  const { quote, author, handle, verified, rating, date } = testimonial;

  return (
    <article
      className={`bg-white rounded-2xl shadow-md hover:shadow-xl transition-all duration-300 p-6 flex flex-col justify-between border-l-4 ${
        isNew
    ...
    >
      {/* Header with rating stars, verification icon, new badge, and date */}
      ...
      {/* Testimonial quote */}
      <p className="text-gray-700 text-base leading-relaxed mb-6 flex-grow">"{quote}"</p>
...
// Form component for submitting a new testimonial
    ...
    <div className="bg-white rounded-2xl shadow-lg p-6 border-2 border-indigo-200">
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div>
          ...
          <StarRating rating={formData.rating} onRatingChange={(rating) => handleChange("rating", rating)} />
        </div>
...
const Testimonials = () => {
  // State for testimonials, form visibility and filtering option
  const [testimonials, setTestimonials] = useState(initialTestimonials);
  const [showForm, setShowForm] = useState(false);
  const [filter, setFilter] = useState("all"); // options: 'all', 'verified', 'recent'
  ...
  // Filter testimonials based on filter state
  ...
  // Calculate average rating for all testimonials
  const averageRating =
  ...
  return (
    <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
      <div className="bg-gradient-to-br from-indigo-50 to-purple-50 rounded-3xl shadow-lg p-8 lg:p-12">
        {/* Header with title, average rating and review count */}
        <div className="text-center mb-12">
          ...
          {/* Filter buttons and Add Review toggle button */}
        ...
        {/* Conditionally render the testimonial submission form */}
        {showForm && (
          <div className="mb-8">
            <TestimonialForm
        ...
        {/* Display testimonial cards grid */}
          ...
          {filteredTestimonials.map((testimonial) => (
            <TestimonialCard
              ...
              isNew={
                ...
                new Date(testimonial.date) > new Date(Date.now() - 24 * 60 * 60 * 1000)
              }
            />
          ))}
        </div>

        {/* Show message if no testimonials match filter */}
